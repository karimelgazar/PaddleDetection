...
class ConvBNLayer(nn.Layer):
    def __init__(self, num_channels, filter_size, num_filters, stride, num_groups=1, act="hard_swish"):
        super().__init__()
        ...
        self.bn = nn.BatchNorm2D(
            num_filters, weight_attr=ParamAttr(regularizer=L2Decay(0.0)), bias_attr=ParamAttr(regularizer=L2Decay(0.0))
        )
        if act == "hard_swish":
...
class DepthwiseSeparable(nn.Layer):
    def __init__(self, num_channels, num_filters, stride, dw_size=3, use_se=False, act="hard_swish"):
        super().__init__()
        self.use_se = use_se
        ...
        self.pw_conv = ConvBNLayer(num_channels=num_channels, filter_size=1, num_filters=num_filters, stride=1, act=act)
...
class AdaptiveAvgPool2D(nn.AdaptiveAvgPool2D):
    def __init__(self, *args, **kwargs):
        ...
        if isinstance(self._output_size, int) and self._output_size == 1:
            self._gap = True
        elif isinstance(self._output_size, tuple) and self._output_size[0] == 1 and self._output_size[1] == 1:
            self._gap = True
        else:
            self._gap = False
...
class LCNet(nn.Layer):
    def __init__(self, scale=1.0, feature_maps=[3, 4, 5], act="hard_swish"):
        ...
        self.conv1 = ConvBNLayer(
            num_channels=3, filter_size=3, num_filters=make_divisible(16 * scale), stride=2, act=act
        )
        ...
        self._out_channels = [ch for idx, ch in enumerate(out_channels) if idx + 2 in feature_maps]
