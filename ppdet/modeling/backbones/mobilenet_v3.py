...
class ConvBNLayer(nn.Layer):
    def __init__(
        ...
        self.conv = nn.Conv2D(
            ...
            padding=padding,
            groups=num_groups,
            weight_attr=ParamAttr(learning_rate=lr_mult, regularizer=L2Decay(conv_decay)),
            bias_attr=False,
        )

        norm_lr = 0.0 if freeze_norm else lr_mult
        param_attr = ParamAttr(
            learning_rate=norm_lr, regularizer=L2Decay(norm_decay), trainable=False if freeze_norm else True
        )
        bias_attr = ParamAttr(
            learning_rate=norm_lr, regularizer=L2Decay(norm_decay), trainable=False if freeze_norm else True
        )
        ...
        if norm_type in ["sync_bn", "bn"]:
            self.bn = nn.BatchNorm2D(out_c, weight_attr=param_attr, bias_attr=bias_attr, use_global_stats=global_stats)
...
class MobileNetV3(nn.Layer):
    ...
    def __init__(
        ...
        if norm_type == "sync_bn" and freeze_norm:
            raise ValueError("The norm_type should not be sync_bn when freeze_norm is True")
